...
create index adm_action_info_01fk
    on adm_action_info (menu_id);
...
comment on table adm_batch_log is '배치작업로그_P';
...
create unique index adm_batch_log_pk
    on adm_batch_log (log_no);
...
create unique index adm_batch_log_m_pk
    on adm_batch_log_m (dd_cmpr_no);
...
create unique index adm_pnt_prvdr_msg_pk
    on adm_pnt_prvdr_msg (prvdr_id, msg_seq);
...
comment on column adm_user_info.login_yn is '시스템로그인여부';
...
comment on column adm_user_login_hist.login_err_rsn is '로그인오류사유';

comment on column adm_user_login_hist.login_ip_enc is '로그인IP암호화';

create index adm_user_login_hist_01fk
    on adm_user_login_hist (user_id);
...
(
    dd_cmpr_no    varchar(14) not null,
    seq           integer     not null,
    cmpr_dd       varchar(8),
    pnt_cd        varchar(30),
    phub_tr_no    varchar(30),
...
create index cl_cmpr_fail_01fk
    on cl_cmpr_fail (dd_cmpr_no);
...
comment on table cl_cmpr_file_info is '클립포인트일대사파일정보';
...
create unique index cl_cmpr_file_info_pk
    on cl_cmpr_file_info (dd_cmpr_no, pt_hb_seqnum);
...
create unique index cl_cmpr_info_pk
    on cl_cmpr_info (dd_cmpr_no);
...
create index ex_ph_deal_info_phub_tr_no_cust_ci_index
...
create index ex_ph_deal_req_info_phub_tr_no_cust_id_index
...
comment on table fp_fmly_supot is '가족지원_P';
...
comment on column fp_fmly_supot.phub_cpn_no is '쿠폰번호';
...
create unique index fp_gifti_show_info_01fk
...
comment on table pg_cmpr_info_d is 'PG일대사D_P';
...
create index pg_cmpr_info_d_01fk
    on pg_cmpr_info_d (dd_cmpr_no);

create unique index pg_cmpr_info_d_pk
    on pg_cmpr_info_d (dd_cmpr_no, seq);
...
create index ph_cls_ag_info_01fk
    on ph_cls_ag_info (cust_id);

create unique index ph_cls_ag_info_pk
    on ph_cls_ag_info (cust_id, cls_id);
...
(
    ...
    mdfy_dt            timestamp,
    mdfy_user_id       varchar(20),
    cpn_cncl_ind       varchar(10) default 'N'::character varying,
    cust_type_cd       varchar(10),
    cust_ctn_enc       varchar(400),
    cust_ctn_sch       varchar(400)[],
    kt_cust_id_enc     varchar(400),
...
comment on table ph_cpn_chng_hist is '쿠폰변경이력_P';
...
comment on column ph_cpn_chng_hist.cpn_cncl_ind is '쿠폰취소경로구분';
...
create index ph_cpn_chng_hist_01fk
    on ph_cpn_chng_hist (phub_cpn_no);

create unique index ph_cpn_chng_hist_pk
    on ph_cpn_chng_hist (phub_cpn_no, hist_seq_no);
...
comment on table ph_cpn_isue_info is '쿠폰발행정보_P';
...
comment on column ph_cpn_isue_info.cpn_cncl_ind is '쿠폰취소경로구분';
...
comment on table ph_mms_send_hist is 'MMS전송이력_P';
...
create index ph_mms_send_hist_01fk
    on ph_mms_send_hist (mms_msg_id);

create unique index ph_mms_send_hist_pk
    on ph_mms_send_hist (mms_msg_no);
...
(
    ...
    mdfy_dt          timestamp,
    mdfy_user_id     varchar(20),
    chrgr_nm_enc     varchar(400),
    chrgr_tel_enc    varchar(400),
...
create unique index ph_pg_cmpn_info_pk_20230216
    on ph_pg_cmpn_info (pg_cmpn_id);
...
create index ph_pg_cprt_prvdr_01fk
    on ph_pg_cprt_prvdr (pg_cmpn_id, cprt_cmpn_id);

create index ph_pg_cprt_prvdr_02fk
    on ph_pg_cprt_prvdr (prvdr_id);

create unique index ph_pg_cprt_prvdr_pk
    on ph_pg_cprt_prvdr (prvdr_id, pg_cmpn_id, cprt_cmpn_id);

alter table ph_pg_cprt_prvdr
    add primary key (prvdr_id, pg_cmpn_id, cprt_cmpn_id);
...
comment on table ph_pnt_cmpn_attr_info is '포인트사용처 속성 정보';

comment on column ph_pnt_cmpn_attr_info.cprt_cmpn_id is '포인트사용처ID';

comment on column ph_pnt_cmpn_attr_info.attr_id is '속성ID(공통코드)';

comment on column ph_pnt_cmpn_attr_info.attr_val is '속성값';

comment on column ph_pnt_cmpn_attr_info.use_yn is '사용여부(Y/N)';

comment on column ph_pnt_cmpn_attr_info.rmrk is '비고';

comment on column ph_pnt_cmpn_attr_info.rgst_dt is '등록일시';

comment on column ph_pnt_cmpn_attr_info.rgst_user_id is '등록자ID';

comment on column ph_pnt_cmpn_attr_info.mdfy_dt is '수정일시';

comment on column ph_pnt_cmpn_attr_info.mdfy_user_id is '수정자ID';
...
comment on column ph_pnt_cmpn_user.mdfy_dt is '수정일시';
...
comment on table ph_pnt_prvd_tmzn_info_hist is '포인트제공처 타입존 정보';

comment on column ph_pnt_prvd_tmzn_info_hist.prvdr_id is '포인트제공처ID';

comment on column ph_pnt_prvd_tmzn_info_hist.tmzn_str_dt is '적용시작일시';

comment on column ph_pnt_prvd_tmzn_info_hist.hist_seq is 'sequence';

comment on column ph_pnt_prvd_tmzn_info_hist.crud_typ is 'crud type(I, U, D)';

comment on column ph_pnt_prvd_tmzn_info_hist.tmzn_end_dt is '적용종료일시';

comment on column ph_pnt_prvd_tmzn_info_hist.tmzn_typ is '시간대유형(EXCL/INCL)';

comment on column ph_pnt_prvd_tmzn_info_hist.use_yn is '사용여부(Y/N)';

comment on column ph_pnt_prvd_tmzn_info_hist.rmrk is '비고';

comment on column ph_pnt_prvd_tmzn_info_hist.rgst_dt is '등록일시';

comment on column ph_pnt_prvd_tmzn_info_hist.rgst_user_id is '등록자ID';

comment on column ph_pnt_prvd_tmzn_info_hist.mdfy_dt is '수정일시';

comment on column ph_pnt_prvd_tmzn_info_hist.mdfy_user_id is '수정자ID';
...
comment on column st_deal_dtl.pay_sale_amt is '할인금액';
...
(
    ...
    rgst_dt      timestamp,
    rgst_user_id varchar(30),
    gender_enc   varchar(400) not null
);
...
alter table st_deal_info_by_gender
    add primary key (deal_dd, gender_enc, service_id, pg_cmpn_id, cprt_cmpn_id);
...
create unique index ph_pnt_cust_info_pk
    on ph_pnt_cust_info (cust_id);

create unique index phpntcustinfo_uk
...
alter table cl_cmpr_sucs
    add constraint cl_cmpr_sucs_merged_pkey
        primary key (dd_cmpr_no, seq, cmpr_dd);
...
create unique index auto_pay_err_hist_merged_pkey
    on auto_pay_err_hist (cust_id, bill_user_key, pay_req_dt);
...
alter table ph_deal_dtl
    add constraint ph_deal_dtl_merged_pkey
        primary key (phub_tr_no, pnt_tr_no, deal_dt);
...
(
    ...
    deal_ind       varchar(20),
    cust_id        varchar(20),
    cncl_yn        varchar(1)  default 'N'::character varying not null,
    pay_method     varchar(20),
    pg_deal_no     varchar(40),
    goods_nm       varchar(100),
    ori_phub_tr_no varchar(20),
...
create index ph_deal_info_merged_cust_id_idx
    on ph_deal_info (cust_id);

create index ph_deal_info_merged_deal_dt_idx
    on ph_deal_info (deal_dt);

create index ph_deal_info_merged_pg_cmpn_id_cprt_cmpn_id_idx
    on ph_deal_info (pg_cmpn_id, cprt_cmpn_id);

alter table ph_deal_info
    add constraint ph_deal_info_merged_pkey
        primary key (phub_tr_no, deal_dt);
...
create index ph_deal_req_dtl_deal_dt_index
    on ph_deal_req_dtl (deal_dt);

alter table ph_deal_req_dtl
    add constraint ph_deal_req_dtl_pk
        primary key (pnt_tr_no, phub_tr_no);
...
(
    prepay_tr_id  varchar(50) not null,
    cust_id       varchar(20) not null,
    cprt_goods_id varchar(20) not null,
    pay_req_tms   integer,
    pay_req_mm    varchar(6)  not null,
    pay_req_pnt   integer,
...
alter table rwrd_goods_prepay_req_hist
    add constraint rwrd_goods_prepay_req_hist_merged_pkey
        primary key (prepay_tr_id, pay_req_mm);
...
create function fnc_querystring(p_query text)
    returns TABLE(value1 character varying, value2 character varying, value3 character varying, value4 character varying, value5 character varying, value6 character varying, value7 character varying, value8 character varying, value9 character varying, value10 character varying)
    language plpgsql
as
$$
...
create function fnc_timestamptotext(ts timestamp without time zone, fmt text) returns text
    immutable
    language plpgsql
as
$$
        BEGIN
                RETURN TO_CHAR(ts, fmt)::text;
        END;
$$;

create function fnc_limit_dummy() returns character varying
    language plpgsql
as
$$
 ...
 begin
  ...
  SELECT 'PH'|| TO_CHAR (NOW (), 'YYMMDD') || LPAD(NEXTVAL('SEQ_PH_ID')::TEXT, 8,'0') into phub_tr_no;



  -- raise notice '허브거래번호: %', phub_tr_no;
...
begin
    ...
    elseif ( new.rgst_dt >= '20260101'::timestamp and new.rgst_dt < '20260201'::timestamp ) then insert into phub.adm_api_if_log_pt_2026y01m (log_no, actn_uri, send_plc, rcv_plc, phub_tr_no, pg_deal_no, rply_cd, rply_msg, rgst_user_id, rgst_dt, req_data_enc, res_data_enc) values (new.log_no, new.actn_uri, new.send_plc, new.rcv_plc, new.phub_tr_no, new.pg_deal_no, new.rply_cd, new.rply_msg, new.rgst_user_id, new.rgst_dt, new.req_data_enc, new.res_data_enc);
...
begin
    ...
    elseif ( new.deal_dt >= '20260101000000' and new.deal_dt < '20260201000000' ) then insert into phub.ph_deal_req_info_pt_2026y01m (phub_tr_no, deal_dt, pg_deal_no, pg_send_po_id, cprt_cmpn_id, pg_cmpn_id, deal_ind, goods_nm, pay_method, deal_stat, cust_id, deal_tkn, ori_phub_tr_no, ori_pg_deal_no, ttl_pnt, ttl_pay_amt, ttl_pnt_amt, ttl_rmnd_amt, err_cd, err_msg, rgst_dt, rgst_user_id, mdfy_dt, mdfy_user_id, auth_limit_dtm, service_id, add_param, inqr_pnt, pltf_ind, ttl_sale_amt, cust_ctn_enc, cust_ctn_sch, cust_ci_enc) values (new.phub_tr_no, new.deal_dt, new.pg_deal_no, new.pg_send_po_id, new.cprt_cmpn_id, new.pg_cmpn_id, new.deal_ind, new.goods_nm, new.pay_method, new.deal_stat, new.cust_id, new.deal_tkn, new.ori_phub_tr_no, new.ori_pg_deal_no, new.ttl_pnt, new.ttl_pay_amt, new.ttl_pnt_amt, new.ttl_rmnd_amt, new.err_cd, new.err_msg, new.rgst_dt, new.rgst_user_id, new.mdfy_dt, new.mdfy_user_id, new.auth_limit_dtm, new.service_id, new.add_param, new.inqr_pnt, new.pltf_ind, new.ttl_sale_amt, new.cust_ctn_enc, new.cust_ctn_sch, new.cust_ci_enc);
